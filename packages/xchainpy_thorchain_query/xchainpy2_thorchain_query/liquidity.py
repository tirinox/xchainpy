...
def get_pool_share(unit_data: UnitData, pool: LiquidityPool,
    ...
    rune = R * units / total
    return LPAmount(
        rune=CryptoAmount(Amount.auto_base(rune, get_base_asset_decimals(base_asset)), base_asset),
        asset=CryptoAmount(Amount.auto_base(asset, pool.thornode_details.decimals), pool.asset)
    )
