...
from xchainpy2_thorchain_query.memo import THORMemo, AUTO_OPTIMIZED, ActionType, Affiliate
from xchainpy2_utils import Asset, get_short_code, AssetRUNE
...
def test_withdraw():
    ...
    m = THORMemo.withdraw('BTC.BTC', 5000, get_short_code(AssetRUNE))
    assert m == THORMemo.withdraw_rune('BTC.BTC', 5000)
    assert m.action == ActionType.WITHDRAW
    assert m.pool == 'BTC.BTC'
    assert m.withdraw_portion_bp == 5000
    assert m.asset == 'r'
    assert m.build() == f'-:BTC.BTC:5000:r'
    assert THORMemo.parse_memo(f'-:BTC.BTC:5000:r') == m
