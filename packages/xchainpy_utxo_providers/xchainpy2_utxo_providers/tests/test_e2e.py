...
async def test_get_tx_data_btc(bitcoin_providers):
    for provider in bitcoin_providers:
        ...
        assert len(tx.inbound_txs) == 2
        assert len(tx.outbound_txs) == 2
        assert tx.type == TxType.TRANSFER
        assert tx.asset == AssetBTC
        assert tx.height == 813956
        in_0 = tx.inbound_txs[0]
        out_1 = tx.outbound_txs[1]
...
async def test_get_tx_data_ltc(litecoin_providers):
    for provider in litecoin_providers:
        ...
        assert len(tx.outbound_txs) == 4
        assert len(tx.inbound_txs) == 2
        assert tx.outbound_txs[3].amount == amt(0.30042918)
        assert tx.outbound_txs[3].asset == AssetLTC
        assert tx.outbound_txs[3].from_address == 'ltc1qq7l2aplmmzqlkacekuauqr6unwfytutuzgkgwq'

        assert tx.inbound_txs[0].address == 'ltc1qp4zm5e0a2s36cw5h5gfr64lzverrxvzn0ef9kg'
        assert tx.inbound_txs[0].amount == amt(0.06260656)
        assert tx.inbound_txs[0].asset == AssetLTC
